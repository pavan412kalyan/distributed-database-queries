...
if __name__ == '__main__':
    try:
        ...
        with testHelper.getOpenConnection(dbname=DATABASE_NAME) as conn:
            ...
            [result, e] = testHelper.testroundrobininsert(MyAssignment, RATINGS_TABLE, 100, 1, 2, conn, '1')
            [result, e] = testHelper.testroundrobininsert(MyAssignment, RATINGS_TABLE, 100, 1, 1, conn, '0')
            if result:
            ...
            MyAssignment.rangeQuery(0, 1, conn, "./rangeResult1.txt")
            # #MyAssignment.rangeQuery(1,4,conn, "./rangeResult.txt")

            # # Calling pointQuery
            # print("Performing Point Query")
            MyAssignment.pointQuery(3, conn, "./pointResult1.txt")
